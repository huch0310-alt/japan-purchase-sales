...
export class EventService {
   ...
   * 发送請求書逾期事件
   */
  notifyInvoiceOverdue(invoice: any) {
    const event: RealtimeEvent = {
   ...
   * 发送商品审核结果事件
   */
  notifyProductApproved(product: any, staffName?: string) {
    const event: RealtimeEvent = {
   ...
   * 发送商品审核拒绝事件
   */
  notifyProductRejected(product: any, reason?: string) {
    const event: RealtimeEvent = {
   ...
   * 发送系统通知
   */
  notifySystem(role: string | string[], title: string, message: string, data?: any) {
    const event: RealtimeEvent = {
   ...
   * 发送给指定角色
   */
  sendToRoles(roles: string[], eventName: string, data: any) {
    if (this.gateway) {
      roles.forEach(role => {
